...
void powermode(void)
{
    ...
    uint8_t tmp,tmp2;
    static const char* POWER_MODE = "POWER MODE";
    
    ret= bmi_read(I2C_NUM_0, &reg_pwr_conf, &tmp,1);
    //ESP_LOGI(POWER_MODE ,"valor de PWR_CONF: %2X \n",tmp);
    if(ret != ESP_OK){
...
// Aqui hay que cambiar a los 4 modos posibles.
...
void low_powermode(void)
{
    ...
    //uint8_t reg_gyr_conf=0x7, val_gyr_conf=0xA9; 
...
void suspended_powermode(void)
{
    ...
    uint8_t reg_value;
    bmi_read(I2C_NUM_0, &reg_pwr_ctrl, &reg_value,1);

    reg_value &= ~(0x06);

    bmi_write(I2C_NUM_0, &reg_pwr_ctrl, &reg_value,1);
    vTaskDelay(1000 /portTICK_PERIOD_MS);

    uint8_t reg_pwr_ctrl2=0x7C;
    uint8_t reg_value2;
    bmi_read(I2C_NUM_0, &reg_pwr_ctrl2, &reg_value2,1);

    reg_value2 &= ~(0x01);

    bmi_write(I2C_NUM_0, &reg_pwr_ctrl2, &reg_value2,1);
    vTaskDelay(1000 /portTICK_PERIOD_MS);   
...
void printAnymotionStatus() {
  uint8_t reg_int_status_0 = 0x1C; // Registro de estado de interrupción general

  uint8_t value_int_status_0;
...
*/
void acc_conf(int range){

    uint8_t addrs = 0x40;
    uint8_t reg_value;
    bmi_read(I2C_NUM_0, &addrs, &reg_value,1);
    reg_value &= ~(0x0f);
...
/**
 * Sets the output data rate and the bandwidth of the Gyroscope in the sensor
*/
void gyr_conf(int range){
    uint8_t addrs = 0x42;
    uint8_t reg_value;
    bmi_read(I2C_NUM_0, &addrs, &reg_value,1);
    reg_value &= ~(0x0f);
...
#define MAX_DATA_POINTS 100 // Número máximo de puntos de datos a almacenar
...
void guardarDatos(BMIData data[], int index, float acc_x, float acc_y, float acc_z, float gyr_x, float gyr_y, float gyr_z) {
    /*data[index].acc_x = acc_x;
    data[index].acc_y = acc_y;
    data[index].acc_z = acc_z;
    data[index].gyr_x = gyr_x;
    data[index].gyr_y = gyr_y;
    data[index].gyr_z = gyr_z;*/

    data[index].acc_x_ms2 = (int16_t) acc_x *(78.4532/32768); 
    data[index].acc_y_ms2 = (int16_t)acc_y *(78.4532/32768);
    data[index].acc_z_ms2 = (int16_t)acc_z *(78.4532/32768);
    data[index].gyr_x = (int16_t)gyr_x *(8.000/32768);
    data[index].gyr_y = (int16_t)gyr_y *(8.000/32768);
    data[index].gyr_z = (int16_t)gyr_z *(34.90659/32768);
    data[index].acc_x_g=  (int16_t)acc_x*(8.000/32768);
    data[index].acc_y_g = (int16_t)acc_y*(8.000/32768);
    data[index].acc_z_g = (int16_t)acc_z*(8.000/32768);
}

// Leer datos de BMI270 una sola vez y guardarlos en un arreglo de tamaño 9, por cada
// posicion se guarda acc_x_ms2 acc_y_ms2 acc_z_ms2 gyr_x gyr_y gyr_z acc_x_g acc_y_g acc_z_g respectivamente
void read_bmi270_data(double data[]){
    uint8_t reg_intstatus=0x03, tmp;
    int bytes_data8 = 12;
    uint8_t reg_data = 0x0C, data_data8[bytes_data8];
    uint16_t acc_x, acc_y, acc_z, gyr_x, gyr_y, gyr_z;
    ...
    if ((tmp & 0b10000000) == 0x80){
        ret = bmi_read(I2C_NUM_0, &reg_data, data_data8, bytes_data8);
        acc_x = (data_data8[1] << 8) | data_data8[0];
        acc_y = (data_data8[3] << 8) | data_data8[2];
        acc_z = (data_data8[5] << 8) | data_data8[4];
        gyr_x = (data_data8[7] << 8) | data_data8[6];
        gyr_y = (data_data8[9] << 8) | data_data8[8];
        gyr_z = (data_data8[11] << 8) | data_data8[10];

        data[0] = (int16_t) acc_x *(78.4532/32768);
        data[1] = (int16_t)acc_y *(78.4532/32768);
        data[2] = (int16_t)acc_z *(78.4532/32768);
        data[3] = (int16_t)gyr_x *(8.000/32768);
        data[4] = (int16_t)gyr_y *(8.000/32768);
        data[5] = (int16_t)gyr_z *(34.90659/32768);
        data[6] =  (int16_t)acc_x*(8.000/32768);
        data[7] = (int16_t)acc_y*(8.000/32768);
        data[8] = (int16_t)acc_z*(8.000/32768);

    }
    //return data;
} 

void read_bmi270_data2(BMIData data[],int* dataCount){
    ...
    //BMIData data[MAX_DATA_POINTS]; // Arreglo para almacenar los datos
    ...
    while (i)
    {
        ...
        { 
            ...
            guardarDatos(data, *dataCount, acc_x, acc_y, acc_z, gyr_x, gyr_y, gyr_z);
            /*
            data[0] = (int16_t)acc_x_ms2*(78.4532/32768); 
            data[1] = (int16_t)acc_y_ms2*(78.4532/32768);
            data[2] = (int16_t)acc_z_ms2*(78.4532/32768);
            data[3] = (int16_t)acc_x*(8.000/32768);
            data[4] = (int16_t)acc_y*(8.000/32768);
            data[5] = (int16_t)acc_z*(8.000/32768);
            data[6] = (int16_t)gyr_x*(34.90659/32768);
            data[7] = (int16_t)gyr_y*(34.90659/32768);
            data[8] = (int16_t)gyr_z*(34.90659/32768);*/
...
void bmi270_execute(double data[])
{
   ...
   // conf_anymotion(10, true, false, true, 200, 0x02);
    ...
    // Llamar a la función read_bmi270_data() para leer y guardar los datos

    read_bmi270_data(data);
    //printf("dataConunt= %d",dataCount);
    //printf("Parte print\n");
    // Utilizar los datos almacenados en el arreglo data según sea necesario
    /*for (int k = 0; k < dataCount; k++) {
        printf("__________________________________________________");
        printf("Dato %d: acc_x = %f m/s2\n", k, data[k].acc_x_ms2);
        printf("Dato %d: acc_y = %f m/s2\n", k, data[k].acc_y_ms2);
        printf("Dato %d: acc_z = %f m/s2\n", k, data[k].acc_z_ms2);
        printf("Dato %d: gyr_x = %f rad/s\n", k, data[k].gyr_x);
        printf("Dato %d: gyr_y = %f rad/s\n", k, data[k].gyr_y);
        printf("Dato %d: gyr_z = %f rad/s\n", k, data[k].gyr_z);
        printf("Dato %d: acc_x = %f g\n", k, data[k].acc_x_g);
        printf("Dato %d: acc_y = %f g\n", k, data[k].acc_y_g);
        printf("Dato %d: acc_z = %f g\n", k, data[k].acc_z_g);
    }*/
    /*printf("Data 0: %f \n",data[0]);
    printf("Data 1: %f \n",data[1]);
    printf("Data 2: %f \n",data[2]);
